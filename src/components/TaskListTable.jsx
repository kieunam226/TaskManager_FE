...
const TaskListTable = ({ tableData }) => {
  const getStatusBadgetColor = (status) => {
    switch (status) {
      case "completed":
          ...
          {tableData.map((task) => (
            <tr key={task._id} className="border-t border-gray-200">
              ...
              <td className="py-4 px-4">
                <span
                  className={`px-2 py-1 test-xs rounded inline-block ${getStatusBadgetColor(
                    task.status
                  )}`}
                >
                  {task.status}
              ...
              <td className="py-4 px-4">
                <span
                  className={`px-2 py-1 test-xs rounded inline-block ${getPriorityBadgeColor(
                    task.priority
                  )}`}
                >
                  {task.priority}
